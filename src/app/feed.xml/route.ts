...
export async function GET() {
  ...
  const entries: Array<{ slug: string; title?: string; date?: string; description?: string }> = [];
  for (const f of files.filter((f) => f.endsWith(".mdx"))) {
    ...
    entries.push({ slug, title: data?.title, date: data?.date, description: data?.description });
  }
